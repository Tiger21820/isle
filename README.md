...
| Binary | reccmp accuracy report | ReproBit verification report |
| --- | --- | --- |
| `CONFIG.EXE` | [CONFIGPROGRESS.HTML](https://isledecomp.github.io/isle/CONFIGPROGRESS.HTML) | [CONFIG target result](https://isledecomp.github.io/isle/report.html#targets) |
| `ISLE.EXE` | [ISLEPROGRESS.HTML](https://isledecomp.github.io/isle/ISLEPROGRESS.HTML) | [ISLE target result](https://isledecomp.github.io/isle/report.html#targets) |
| `LEGO1.DLL` | [LEGO1PROGRESS.HTML](https://isledecomp.github.io/isle/LEGO1PROGRESS.HTML) | [LEGO1 target result](https://isledecomp.github.io/isle/report.html#targets) |

The reccmp reports compare every recompiled function with the retail binary.
The ReproBit report records the cold rebuild that reproduced all three
binaries byte for byte and links its full `report.json` record; both are
regenerated by every verified push to `master`.
...
`report.html` plus its full `report.json` record; the reports are also
published to the progress pages linked under [Status](#status). Every run
compares its reccmp scores and its ReproBit intervention cost with the
`master` release and prints the deltas in the job log. The ordinary CMake
